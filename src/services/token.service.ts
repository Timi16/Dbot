...
import type { TokenDetails, DexToolResponse, ResponseObject } from '../types/token.types.js'
...
export async function getNativeTokenUsdPrice(chain: ChainKey): Promise<number> {
  ...
  if (!res.ok)
    throw new Error(
      `CoinGecko request failed (${res.status}): ${await res.text()}`
    )
  ...
  if (typeof price !== 'number') {
    throw new Error(
      `Invalid response from CoinGecko: missing ${coingeckoId}.usd`
    )
  }
...
export interface TokenOnChainData {
  ...
  symbol: string
  decimals: number
  totalSupply: string // as a string to avoid JS number overflow
}

/**
 * Fetch ERC-20 token metadata directly from the blockchain.
 */
export const getTokenDataFromChain = async (
  tokenAddress: string,
  ...
  try {
    ...
    if (data.pairs && data.pairs.length > 0) {
      ...
      const result: TokenDetails = {
        ...
        telegramUrl: pair.info?.socials?.find(
          (s) => s.type === 'telegram'
        )?.url || '',
        twitterUrl: pair.info?.socials?.find(
          (s) => s.type === 'twitter'
        )?.url || '',
...
export const getTokenDetails_DEXTOOLS = async (
  ...
  try {
    ...
    if (_data.data) {
      ...
      if (_data2.data) {
        data = _data2.data[0] as DexToolResponse
      } else if (_data2.results) {
        data = _data2.results[0] as DexToolResponse
      } else {
        return null
      }
    }
    ...
    const result: TokenDetails = {
      ...
      change: {
        m5: calculatePercentageChange(
          data.periodStats?.['5m']?.price?.usd?.last || 0,
          data.price
        ),
        h1: calculatePercentageChange(
          data.periodStats?.['1h']?.price?.usd?.last || 0,
          data.price
        ),
        h24: calculatePercentageChange(
          data.periodStats?.['24h']?.price?.usd?.last || 0,
          data.price
        ),
      },
...
const KNOWN_TOKEN_DETAILS: Record<string, { socials: { telegramUrl: string; websiteUrl: string; twitterUrl: string } }> = {
...
export const getCustomTokenDataForEvmChainUsingUniSwapV30g = async (
  tokenAddress: string,
  ...
  try {
    ...
    try {
      const nativeRes = await fetch(
        `https://screener.deserialize.xyz/tokens/${nativeAddress}`
      )
      if (nativeRes.ok) {
    ...
    const calculateChangeForInterval = async (
      interval: string,
      timeMs: number
    ): Promise<number> => {
      try {
    ...
    console.error(
      'Error in getCustomTokenDataForEvmChainUsingUniSwapV30g:',
      error
    )
    return null
  }

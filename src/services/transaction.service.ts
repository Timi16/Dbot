...
import type { SVMChainWallet, EVMChainWallet, SVMVM, EVMVM } from '@demetacode/multi-vm-wallet'
import { PublicKey } from '@solana/web3.js'
import { prisma } from '../models/prisma.client.js'
import { walletService } from './wallet.service.js'
import { normalizeChainKey, getChainConfig, type ChainKey } from '../config/index.js'
import type {
    TransferParams,
    SwapParams,
    TransactionDetails,
...
export class TransactionService {
     ...
     */
    async sendNative(params: TransferParams): Promise<TransactionDetails> {
        ...
        if (walletBalance.nativeBalance.formatted < amount) {
            throw new InsufficientBalanceError(
                `${amount} ${walletBalance.nativeSymbol}`,
                `${walletBalance.nativeBalance.formatted} ${walletBalance.nativeSymbol}`
            )
        }
     ...
     */
    async sendToken(
        params: TransferParams & { tokenAddress: string }
        ...
        try {
            ...
            if (wallet.chain === 'SVM') {
                ...
                const svmVM = (svmWallet as unknown as { vm: SVMVM }).vm
                ...
                // ✅ get the VM and call VM.getTokenInfo
                const evmVM = (evmWallet as unknown as { vm: EVMVM }).vm
     ...
     * Swap tokens
     */
    async swap(params: SwapParams): Promise<TransactionDetails> {
        ...
        try {
            ...
            if (balance.formatted < amount) {
                throw new InsufficientBalanceError(
                    ...
                    `${balance.formatted} ${fromToken.symbol}`
                )
            }
    ...
    async getTransaction(hash: string): Promise<TransactionDetails | null> {
        ...
        return transaction as TransactionDetails | null
    }
    ...
    async updateTransactionStatus(
        ...
        const transaction = await prisma.updateTransactionStatus(
            hash,
            status,

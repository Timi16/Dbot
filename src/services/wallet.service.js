 ...
 */
import { SVMVM, SVMChainWallet, EVMVM, EVMChainWallet, VM, } from '@demetacode/multi-vm-wallet';
import { Connection, PublicKey } from '@solana/web3.js';
import { JsonRpcProvider as EthersJsonRpcProvider } from 'ethers';
import * as bip39 from '@scure/bip39';
import { wordlist } from '@scure/bip39/wordlists/english.js';
import { prisma } from '../models/prisma.client.js';
import { getChainConfig, normalizeChainKey } from '../config/index.js';
import { encryptSeed, decryptSeed, NotFoundError, ValidationError, } from '../utils/index.js';
export class WalletService {
     ...
     */
    async createUserWallets(userId, pin) {
        // Generate mnemonic
     ...
     */
    async getUserWalletWithSeed(userId, chainKey) {
     ...
     */
    async getUserWallet(userId, chainKey) {
     ...
     * Get wallet balance
     */
    async getWalletBalance(userId, chainKey) {
        const normalized = normalizeChainKey(chainKey);
        ...
        if (wallet.chain === 'SVM') {
            ...
            nativeBalance = await SVMVM.getNativeBalance(new PublicKey(wallet.address), connection);
        }
        else {
    ...
    async getAllBalances(userId) {
        ...
        for (const wallet of wallets) {
            try {
                const balance = await this.getWalletBalance(userId, wallet.chainKey);
     ...
     */
    async getWalletInstance(userId, chainKey, pin) {
        const normalized = normalizeChainKey(chainKey);
     ...
     */
    async getSeedPhrase(userId, pin) {
        const wallets = await prisma.wallet.findMany({
            where: { userId },
            take: 1,
        });

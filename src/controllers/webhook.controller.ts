...
async function handleMainFlow(phone: string, message: string): Promise<string> {
    try {
        ...
        // Check if message is a contract address (before other checks)
        const trimmedMessage = message.trim()
        if (isContractAddress(trimmedMessage)) {
            return await handleTokenLookup(phone, user.id, trimmedMessage)
        }
...
async function handleTokenLookup(
    ...
    try {
        ...
        let specificChain: string = 'ethereum' // default

        if (chainType === 'solana') {
            // Try DexScreener first for Solana
            tokenDetails = await getTokenDetails_DEXSCREENER(contractAddress)

            // Fallback to DexTools if DexScreener fails
            if (!tokenDetails) {
            ...
            // For EVM, try different sources based on detection
            tokenDetails = await getTokenDetails_DEXSCREENER(contractAddress)
            ...
            // Special handling for 0G chain (not on DexScreener/DexTools)
            if (!tokenDetails) {
            ...
            // If we got token details, map the chain
            if (tokenDetails) {
            ...
            // Get user's wallet for the detected chain
        ...
        // Store token info in session for quick swap access
        ...
        // Show user's relevant wallet address
        ...
        message += `💡 *Want to trade this token?*\n`
        message += `Reply *"buy"* to purchase or *"swap"* to exchange!`
...
async function handleSwapTokens(
  ...
  try {
    ...
    // If user says "buy/swap" right after viewing a token, prefill swap-to with that token
    if (lastViewedToken) {
      await sessionService.updateSession(phone, {
        // Move into the swap flow; pick the step you use for picking "from" token next
        currentStep: MainStep.SWAP_TOKENS_FROM, // or SWAP_TOKENS_CHAIN if you gate by chain first
        context: {
          ...(session.context ?? {}),
          ...entities,
          // Pre-fill destination token details from lastViewedToken
          toToken: lastViewedToken.symbol,
          toTokenAddress: lastViewedToken.address,
          toTokenName: lastViewedToken.name,
          chain: lastViewedToken.chain,
          lastViewedToken, // keep it for later if needed
        } as SessionContext,
      });

      return `Great! What token would you like to swap *from*?\n\nFor example: SOL, ETH, USDC, etc.`;
    }
    ...
    // Otherwise, continue the guided flow
    await sessionService.updateSession(phone, {
...
async function handleSwapFromTokenStep(
    ...
    // Check user's balance for this token
    try {
        ...
        return `How much ${fromToken} would you like to swap for ${toTokenDisplay}?${balanceMessage}`
    } catch (error) {
        ...
        const toTokenDisplay = context.toTokenSymbol || context.toToken
        return `How much ${fromToken} would you like to swap for ${toTokenDisplay}?`
    }
}
    ...
    try {
        ...
        if (context.fromToken === balance.nativeSymbol) {
            ...
            if (amountNum > availableBalance) {
                return `❌ Insufficient balance. You have ${availableBalance} ${context.fromToken} available.\n\nPlease enter a smaller amount:`;
            }
        }
